	...
	function handlePlayTrack(event) {
		...
		if (videoId) {
			currentTrack.update(t => ({
				...
				albumImage: track.album?.images?.[0]?.url || track.imageUrl || '/default-album.png', // ✅ 순서 조정
				englishTrackName: track.englishTrackName || track.name, // ✅ 추가
				englishArtistName: track.englishArtistName || track.artist // ✅ 추가
			}));
